use std::{mem, slice, sync::Arc, time::Duration};
...
use super::{
    ...
    loader::DebugLoaderEXT,
    resource::{BufferIndex, Resource},
    ImguiPushConstant, Swapchain, TKQueue,
};
...
impl VulkanContext {
    ...
    pub fn new(event_loop: &EventLoop<()>, max_frames_in_flight: usize, is_imgui: bool, window: &winit::window::Window) -> Self {
        unsafe {
    ...
    pub fn recreate_swapchain(&mut self, window: &winit::window::Window) {
        let window_extent_physical = window.inner_size();
    ...
    pub fn process_imgui_event(&mut self, event: &Event<()>, window: &winit::window::Window) {
        self.imgui.as_mut().unwrap().process_event_imgui(window, event);
    }
...
impl ImguiContext {
    fn new(
        ...
        unsafe {
            ...
            let index_buffers = buffer_builder
                ...
                .build_resource(buffer_storage, vk::CommandBuffer::null());
    ...
    pub fn get_draw_instance(&mut self, window: &winit::window::Window) -> &mut imgui::Ui {
    ...
    pub fn render(
        ...
        unsafe {
            ...
            self.device
                .cmd_bind_index_buffer(cmd, res.get_buffer_ref(index_index).buffer, 0, vk::IndexType::UINT16);
            self.device
                .cmd_bind_vertex_buffers(cmd, 0, &[res.get_buffer_ref(vertex_index).buffer], &[0]);
