...
pub fn create_sampler(device: &ash::Device, filter: vk::Filter, sampler_adress_mode: vk::SamplerAddressMode) -> vk::Sampler {
    let sampler_info = vk::SamplerCreateInfo::default().address_mode_u(sampler_adress_mode).address_mode_v(sampler_adress_mode).address_mode_w(sampler_adress_mode).mag_filter(filter).min_filter(filter);

    unsafe { device.create_sampler(&sampler_info, None).unwrap() }
}

pub fn create_cmd(device: &ash::Device, pool: CommandPool) -> vk::CommandBuffer {
    let cmd_info = vk::CommandBufferAllocateInfo::default().command_pool(pool).level(CommandBufferLevel::PRIMARY).command_buffer_count(1);
...
pub fn create_fence(device: &ash::Device) -> vk::Fence {
    unsafe { device.create_fence(&vk::FenceCreateInfo::default().flags(vk::FenceCreateFlags::SIGNALED), None).unwrap() }
}
...
pub fn create_unlinked_shader(context: &VulkanContext, shader_loader: ShaderLoaderEXT, path: String, shader_stage: vk::ShaderStageFlags, descriptor_layout: Vec<vk::DescriptorSetLayout>, push_constants: Vec<vk::PushConstantRange>) -> vk::ShaderEXT {
...
pub fn create_bindless_layout(device: &ash::Device, binding: u32, descriptor_type: Vec<vk::DescriptorType>, debug_loader: &DebugLoaderEXT, name: CString) -> vk::DescriptorSetLayout {
    ...
    let layout_info = vk::DescriptorSetLayoutCreateInfo::default().bindings(&bindings).flags(vk::DescriptorSetLayoutCreateFlags::UPDATE_AFTER_BIND_POOL).push_next(&mut binding_flags);

    unsafe {
        let layout = device.create_descriptor_set_layout(&layout_info, None).unwrap();

        debug_loader.set_debug_util_object_name_ext(vk::DebugUtilsObjectNameInfoEXT::default().object_handle(layout).object_name(&name)).unwrap();
        layout
    }
}
pub fn create_shader(device: &ash::Device, path: String) -> vk::ShaderModule {
    ...
    let vec_u32: Vec<u32> = data.chunks_exact(4).map(|chunk| u32::from_le_bytes([chunk[0], chunk[1], chunk[2], chunk[3]])).collect();

    let shader_info = vk::ShaderModuleCreateInfo::default().code(&vec_u32);
    unsafe { device.create_shader_module(&shader_info, None).unwrap() }
}

pub fn create_shader_ext(context: &VulkanContext, shader_loader: ShaderLoaderEXT, path: String, shader_stage: vk::ShaderStageFlags, descriptor_layout: vk::DescriptorSetLayout) -> vk::ShaderEXT {
...
pub fn slice_as_u8_vec<T: Sized>(data: &[T]) -> &[u8] {
...
pub fn transition_image_shader_only(device: &ash::Device, cmd: vk::CommandBuffer, image: &mut AllocatedImage, src_layout: vk::ImageLayout, src_access: vk::AccessFlags) {
    let mut barrier = vec![init::image_barrier_info(
        image.image,
        src_layout,
...
pub fn end_cmd_and_submit(device: &ash::Device, cmd: vk::CommandBuffer, queue: TKQueue, signal_done: Vec<vk::Semaphore>, wait_semp: Vec<vk::Semaphore>, done_fence: vk::Fence) {
    unsafe {
        ...
        let mut submit_info = SubmitInfo::default().command_buffers(&cmds).signal_semaphores(&signal_done).wait_semaphores(&wait_semp);
...
pub fn present_submit(swapchain_loader: &swapchain::Device, graphic: TKQueue, swapchain: vk::SwapchainKHR, swapchain_index: u32, wait_semp: Vec<vk::Semaphore>) -> VkResult<bool> {
    unsafe {
        swapchain_loader.queue_present(
            graphic.queue,
            &vk::PresentInfoKHR::default().swapchains(&[swapchain]).wait_semaphores(&wait_semp).image_indices(&[swapchain_index as u32]),
        )
    }
}
...
pub fn copy_to_image_from_buffer(device: &ash::Device, cmd: vk::CommandBuffer, dst_image: &AllocatedImage, buffer: (vk::Buffer, &vk_mem::Allocation)) {
    unsafe {
        ...
        let image_subresource_layer = vk::ImageSubresourceLayers::default().aspect_mask(vk::ImageAspectFlags::COLOR).base_array_layer(0).layer_count(1).mip_level(0);
...
pub fn copy_to_image_array_from_buffer(device: &ash::Device, cmd: vk::CommandBuffer, dst_image: &mut AllocatedImage, buffer: &mut (vk::Buffer, vk_mem::Allocation), layers: u32) {
    unsafe {
        let image_extent = vk::Extent3D { width: dst_image.extent.width, height: dst_image.extent.height, depth: 1 };

        let image_subresource_layer = vk::ImageSubresourceLayers::default().aspect_mask(vk::ImageAspectFlags::COLOR).base_array_layer(0).layer_count(layers).mip_level(0);

        let copy_region = vec![BufferImageCopy::default().buffer_offset(0).buffer_image_height(0).buffer_row_length(0).image_extent(image_extent).image_subresource(image_subresource_layer)];
...
pub fn copy_to_image_from_image(device: &ash::Device, cmd: vk::CommandBuffer, src_image: &AllocatedImage, dst_image: &AllocatedImage, extent: vk::Extent2D) {
    ...
    unsafe {
        ...
        let sub_resource_layer = vk::ImageSubresourceLayers::default().aspect_mask(ImageAspectFlags::COLOR).base_array_layer(0).layer_count(1).mip_level(0);

        let offsets = [Offset3D::default().x(0).y(0).z(0), Offset3D { x: extent.width as i32, y: extent.height as i32, z: 1 }];
        let image_blit = vk::ImageBlit::default().dst_offsets(offsets).dst_subresource(sub_resource_layer).src_offsets(offsets).src_subresource(sub_resource_layer);
        ...
        src_barrier = src_barrier.old_layout(ImageLayout::TRANSFER_SRC_OPTIMAL).new_layout(ImageLayout::GENERAL).src_access_mask(AccessFlags::TRANSFER_READ).dst_access_mask(AccessFlags::SHADER_WRITE);
...
pub fn generate_mip_levels_array(device: &ash::Device, cmd: vk::CommandBuffer, image: &AllocatedImage, grid: u32, layers: u32, miplevels: u32, filter: vk::Filter) {
    ...
    let base_layer = 0;
    let mut subrange_src = vk::ImageSubresourceLayers::default().aspect_mask(vk::ImageAspectFlags::COLOR).base_array_layer(base_layer).layer_count(1);

    let mut subrange_dst = vk::ImageSubresourceLayers::default().aspect_mask(vk::ImageAspectFlags::COLOR).base_array_layer(base_layer).layer_count(1);
    ...
    for layer in 0..layers {
        ...
        for i in 0..miplevels - 1 {
            ...
            let blit = vk::ImageBlit::default().dst_offsets(offset_dst).dst_subresource(subrange_dst).src_offsets(offset_src).src_subresource(subrange_src);
