...
use ash::vk::{
    self, BufferUsageFlags, DebugUtilsObjectNameInfoEXT, DescriptorType, Extent2D, Extent3D,
    Handle, ImageLayout, ImageSubresourceRange, ImageUsageFlags, MemoryPropertyFlags,
};
use vk_mem::Alloc;
...
impl Resource {
    ...
    pub fn create_buffer_non_descriptor(
        ...
        assert!(
            buffer_type == BufferType::Index || buffer_type == BufferType::Vertex,
            "Use regular create buffer"
        );
    ...
    pub fn create_depth_image(
        ...
        unsafe {
            ...
            let view_info =
                init::image_view_info(depth_image.0, format, vk::ImageAspectFlags::DEPTH);
    ...
    pub fn create_texture_image(
        ...
        let (image_info, alloc_info) =
            init::image_info(extent, 4, memory, vk::Format::R8G8B8A8_UNORM, usage);
        ...
        unsafe {
            let texture_image = self
                ...
                .create_image(&image_info, &alloc_info)
                .unwrap();
            let view_info = init::image_view_info(
                texture_image.0,
                image_info.format,
            ...
            let sampler = util::create_sampler(
                &self.device,
                vk::Filter::LINEAR,
            ...
            util::copy_to_image_from_buffer(
                &self.device,
                self.cmd,
                &image,
            ...
            let image_descriptor =
                init::image_descriptor_info(image.layout, image.view, image.sampler);
            ...
            util::debug_object_set_name(
                &self.debug_loader,
                image.sampler.as_raw(),
                vk::ObjectType::SAMPLER,
                sampler_n,
            );

            self.temp[self.frame_index as usize]
                .staging_buffers
                .push((staging_buffer, staging_alloc));
    ...
    pub fn create_texture_array(&mut self, data: TextureArray, name: String) -> AllocatedImage {
        ...
        unsafe {
            ...
            util::copy_to_image_array_from_buffer(
                &self.device,
                self.cmd,
                &mut image,
                &mut staging,
            ...
            util::generate_mip_levels_array(
                &self.device,
                self.cmd,
                &image,
                data.grid,
    ...
    pub fn create_storage_image(
        ...
        unsafe {
            ...
            self.device
                .queue_wait_idle(self.graphic_queue.get_queue())
                .unwrap();

            let image_descriptor = init::image_descriptor_info(
                alloc_image.layout,
                alloc_image.view,
                alloc_image.sampler,
            );
    ...
    pub fn write_to_buffer_local(
        ...
        let buffer_region = vk::BufferCopy::default()
            .dst_offset(0)
            .src_offset(0)
            .size(data.len() as u64);
        unsafe {
            self.device
                .cmd_copy_buffer(cmd, staging.0, buffer.buffer, &[buffer_region]);
        }
    ...
    pub fn resize_buffer(&mut self, resize_buffer: &mut AllocatedBuffer, new_size: u64) {
        ...
        unsafe {
            let new_buffer = self
                ...
                .unwrap();
            self.allocator
                .destroy_buffer(resize_buffer.buffer, &mut resize_buffer.alloc);
    ...
    pub fn resize_image(&mut self, alloc_image: &mut AllocatedImage, new_extent: vk::Extent2D) {
        assert!(
            alloc_image.alloc.is_some(),
            "Resource is not created with VMA"
        );

        unsafe {
            /*Destroy */
            self.allocator
                .destroy_image(alloc_image.image, alloc_image.alloc.as_mut().unwrap());
            ...
            let image_view_info =
                init::image_view_info(image.0, alloc_image.format, vk::ImageAspectFlags::COLOR);
            ...
            if alloc_image.layout == vk::ImageLayout::GENERAL {
                ...
                self.device
                    .queue_wait_idle(self.graphic_queue.queue)
                    .unwrap();
            }
            let image_descriptor = init::image_descriptor_info(
                alloc_image.layout,
    ...
    pub fn resize_buffer_non_descriptor(
        &mut self,
        resize_buffer: &mut AllocatedBuffer,
        new_size: u64,
    ) {
        let buffer_info = vk::BufferCreateInfo::default()
            .sharing_mode(vk::SharingMode::EXCLUSIVE)
